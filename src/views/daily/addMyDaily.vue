<template>
  <div>
    <Form ref="formData" :model="formData" :label-width="80" style="width: 1000px; margin:0 auto;">
      ...
      <FormItem v-for="(item, index) in formData.items" :key="index" :label="String(index + 1)"
        ...
        <Row>
          ...
          <Select v-model="item.workCategory" style="margin-left: 10px;">
            <Option v-for="item in workCategoryList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
          </Col>
          <Col span="4" offset="1">
      ...
      <FormItem>
        <Row>
          <Col span="18">
          <Input v-model="formData.ps" type="textarea" placeholder="备注" :autosize="{minRows: 3, maxRows: 5}"
            style="width: 100%" />
          </Col>
        </Row>
      </FormItem>
