  ...
  <div style="padding: 50px">
    ...
    <Tabs type="card">
      <TabPane label="详情">
        <Row>
          <Col span="10">
            <Table size="small" :columns="tableTitle" :data="weekDetail"></Table>
          </Col>
          <Col span="14">
...
export default {
  ...
  mounted() {
    this.$http
      ...
      .then(data => {
        ...
        this.updateFlag = infos.updateFlag;
  ...
  data() {
    return {
      ...
      updateFlag: false,
      weekData: {
        monday: { date: "", workInfos: [], ps: "", sumCost: 0 },
        tuesday: { date: "", workInfos: [], ps: "", sumCost: 0 },
        wednesday: { date: "", workInfos: [], ps: "", sumCost: 0 },
        thursday: { date: "", workInfos: [], ps: "", sumCost: 0 },
        friday: { date: "", workInfos: [], ps: "", sumCost: 0 },
        saturday: { date: "", workInfos: [], ps: "", sumCost: 0 },
        sunday: { date: "", workInfos: [], ps: "", sumCost: 0 }
      },
      weekOptions: {
        shortcuts: [
          {
      ...
      tableTitle: [
        { title: "星期", key: "weekday", width: "60" },
        { title: "日期", key: "date" },
        { title: "编码", key: "coding", width: "60" },
        { title: "测试", key: "testing", width: "60" },
        { title: "文档编写", key: "documentWriting", width: "60" },
        { title: "自学", key: "selfStudying", width: "60" },
        { title: "翻译", key: "translate", width: "60" },
        { title: "准备工作", key: "useless", width: "60" }
      ]
    };
  },
  methods: {
    ...
    save() {
      ...
      if (this.updateFlag) {
        ...
        this.$http
          .post("/api/myDaily/importOriginData", { infos: [infos] }, {})
          .then(response => {
            this.goBack();
          });
      }
    ...
    openConfirmDailog(infos) {
      this.$Modal.confirm({
        title: "更新周报确认",
        content:
          "<p>Are you sure you want to update a current EXISTED WEEK DAILY RECORD?</p>",
        loading: true,
        onOk: () => {
          this.$http
            .put(
              ...
              {
                infos: infos,
                weeks: this.$router.history.current.params.infos.weeks
              },
              {}
            )
            .then(response => {
