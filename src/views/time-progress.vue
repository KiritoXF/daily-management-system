        ...
        <Row :gutter="10">
            ...
            <div>
                <Card class="progress-card">
                    ...
                    <label class="card-bottom-text">{{ getYearRestDays() }} days until the end of
                        {{ this.currentDate.getFullYear() }}</label>
                </Card>
            </div>
            </Col>
            <Col :span="12">
            <div>
                <Card class="progress-card">
                    ...
                    <div>
                        <Progress text-inside :stroke-width="64" :percent="transProgress(getMonthProgress())" />
                    </div>
                    <label class="card-bottom-text">{{ getMonthRestDays() }} days until the end of
                        {{ this.months[this.currentDate.getMonth()] }}</label>
                </Card>
            </div>
            </Col>
        </Row>
    ...
    export default ({
        ...
        methods: {
            ...
            getYearProgress() {
                ...
                return ((dayCount - this.getYearRestDays() + 1) * 24 + this.getCurrentDayPastHours()) /
                    (dayCount * 24);
            },
            // 距离这年结束还剩多少天
            getYearRestDays() {
                const currentYear = this.currentDate.getFullYear();
                const pastDay = Math.ceil((this.currentDate - new Date(currentYear.toString())) / 86400000);
                return (this.isLeapYear() ? 366 : 365) - pastDay;
            },
            // 刷新页面
            refresh() {
                location.reload();
